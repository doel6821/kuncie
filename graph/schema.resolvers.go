...
func (r *mutationResolver) UpdateProduct(ctx context.Context, input *model.NewProduct) (*model.ResponseUpdated, error) {
	_, err := productService.UpdateProduct(input)
	if err != nil {
		return nil, err
	}
	return &model.ResponseUpdated{
		Status: "Success",
	}, nil
}
...
func (r *mutationResolver) UpdatePromo(ctx context.Context, input *model.NewPromo) (*model.ResponseUpdated, error) {
	_, err := promoService.UpdatePromo(input)
	if err != nil {
		return nil, err
	}
	return &model.ResponseUpdated{
...
func (r *queryResolver) Cartlist(ctx context.Context) ([]*model.Cart, error) {
	carts, err := cartService.GetAllCart()
	if err != nil {
		return nil, err
	}
...
var (
	...
	cartService     = service.NewCartServiceImpl(cartRepo, productRepo)
